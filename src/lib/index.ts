...
import {
  ...
  isInput,
  isSvg,
  clearTransformAndTransition,
  getNumber,
  adjustTextNodeHeight,
} from "./helpers";
...
const processOneElement = async (element: HTMLElement, styles: TargetProps, parentStyles: TargetProps) => {
   ...
   */
  result.children = [];
  let childNodes: (ChildNode | PesudoElt | PesudoInputText)[] =
    element.tagName !== "TEXTAREA" ? Array.from(element.childNodes ?? []) : [];

  // 合并伪元素数组和输入框文字
  childNodes = childNodes.concat(pseudoElts as any).concat(textNode!);
  const convertedChildren = await Promise.allSettled(
    childNodes.map(async (childNode) => {
      ...
      if (childNode.nodeType === Node.ELEMENT_NODE) {
        ...
        const _rect = adjustTextNodeHeight(range, styles) || rect;
        range.detach();
        child = transformText(
          childNode as any,
          {
            ...
            x: `${_rect.x - bound.x}px`,
            y: `${_rect.y - bound.y}px`,
            left: `${_rect.left}px`,
        ...
        child = transformText(
          childNode.node,
          childNode.styles,
          Object.assign(childNode.styles, { isChildNodeStretched: styles.isChildNodeStretched })
        );
      }
      return child;
    })
  ...
  result.children = convertedChildren
    ...
    .map((item) => (item as PromiseFulfilledResult<TargetNode>).value);
  //子元素排序
